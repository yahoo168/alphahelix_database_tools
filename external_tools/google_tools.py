...
# 替换 'path_to_your_service_account_file.json' 为你的服务账号文件的路径
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = "/Users/yahoo168/Desktop/alphahelixauto_admin.json"

# 待調用函數: 用於上傳file or file_name物件類型的檔案
def _upload_to_bucket(bucket_name, blob_data):
...
def upload_to_google_cloud_storage(bucket_name, blob_meta_list):
    blob_url_dict = {}
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future_to_blob_name = {
            executor.submit(_upload_to_bucket, bucket_name, blob_meta): blob_meta["blob_name"]
...
def get_blob_list_in_folder(bucket_name, folder_name):
...
def download_blob(bucket_name, blob_name, file_path):
    storage_client = storage.Client()
    bucket = storage_client.bucket(bucket_name)
    blob = bucket.blob(blob_name)
