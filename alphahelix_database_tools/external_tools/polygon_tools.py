...
def save_stock_shares_outstanding_from_Polygon(API_key, ticker_list, date_list):
...
def save_stock_universe_ticker_from_polygon(API_key, universe_name, date_list):    
    ...
    for date in date_list:
        ...
        s = pd.Series(ticker_list)
        # 因MDB不允許key為int，故改為str（待改：改在這層合理嗎？）
        s.index = s.index.astype(str)
        data_dict[date] = s.to_dict()
